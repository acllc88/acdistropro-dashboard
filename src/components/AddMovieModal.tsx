...
interface AddMovieModalProps {
  channels: Channel[];
  onClose: () => void;
  onAdd: (movie: { title: string; genre: string; year: number; rating: number; duration: string; description: string; language: string; poster: string; channelId: string | null }) => void;
}
              ...
              {emojis.map(e => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg flex items-center justify-center text-lg transition-all ${
                    poster === e ? 'bg-amber-100 ring-2 ring-amber-500 scale-110' : 'bg-gray-50 hover:bg-gray-100'
                  }`}
                >
                  {e}
                </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., 120m or 45m/ep"
                className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
              />
            </div>
          </div>
